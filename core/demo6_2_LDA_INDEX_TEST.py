...
#     corpus_lda_simi_matrix = similarities.MatrixSimilarity(lda_model[corpus_tfidf])
    print "LDA MODEL BEGIN LOAD"
#     print index
    compare_file = "abstract9755578/pat_lda2vec.txt"
    if (os.path.exists(compare_file)):
        raise Exception("Please delete compare_file first") 
    ldawriter = open(compare_file, "a")
    file_index = 0
    current = 0
    for file_index in range (0, 100):
        index_file = "index_lda/sim.index." + str(file_index)
        if(os.path.exists(index_file)):
            ...
            inferred_vector_lda = lda_model[inferred_vector_tfidf]
            print inferred_vector_lda
            test_lda_simi = index[inferred_vector_lda]
            length = len(list(test_lda_simi))
            print length
            for i in range (0, length):
                rank = list(test_lda_simi)[i];
                line = i + current
                content = str(line) + "," + str(rank) 
                if(i%10000 == 0):
                    print "Document: " + str(line)
                    print "Content: " + content 
                ldawriter.write(content + "\n")
                i = i + 1
            print("LDA END")
