...
if (os.path.exists("model/pat_tfidf.model")):
    ...
    print "TF/IDF MODEL BEGIN LOAD"
#     print utils.get_max_id(corpus)
#     corpus_tfidf = tfidf_model[corpus]
    
#     index = ShardedCorpus.load("index/sim.index.0").
#     print index
    compare_file = "abstract9754858/pat_tfidf2vec.txt"
    if (os.path.exists(compare_file)):
        raise Exception("Please delete compare_file first") 
    tfwriter = open(compare_file, "a")
    file_index = 0
    current = 0
    for file_index in range (0, 100):
        index_file = "index/sim.index." + str(file_index)
        if(os.path.exists(index_file)):
            index = Similarity.load(index_file)
            print index
            print "TF/IDF SIMS BEGIN COMPARE: Index" + str(file_index)
            inferred_vector = dictionary.doc2bow(abstract9754858.lower().split())
            print inferred_vector
            inferred_vector_tfidf = tfidf_model[inferred_vector]
            print inferred_vector_tfidf
            test_tfidf_simi = index[inferred_vector_tfidf]
            length = len(list(test_tfidf_simi))
            print length
            for i in range (0, length):
                rank = list(test_tfidf_simi)[i];
                line = i + current
                content = str(line) + "," + str(rank) 
                if(i % 10000 == 0):
                    print "Document: " + str(line)
                    print "Content: " + content 
                tfwriter.write(content + "\n")
                i = i + 1
            print("TFIDF END")
